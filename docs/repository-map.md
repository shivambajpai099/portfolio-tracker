...
| `(tabs)/index.tsx` | `/` | **Dashboard** — portfolio totals, geographic split, allocation donut/list with geo filter, onboarding modal |
...
|---|---|---|
| `quote.ts` | `GET /api/quote?symbols=A,B` | Yahoo Finance quote proxy; in-process cache (20 min); retry across query1/query2; India fallback via `nse-bse-api`; Finnhub fill for remaining missing symbols; input sanitisation (max 50 symbols, validated pattern) |
...
| File | Description |
|---|---|
| `calculations.ts` | **All pure financial math** — FX helpers (`toINR`, `toUSD`, `convert`); holding-level cost/value; `calcPortfolioTotals`; `calcSymbolAllocations` (with allocation basis + cash toggle); `calcGeographicSplit` |
...
|---|---|---|
| `EXPO_PUBLIC_SUPABASE_URL` | `supabaseClient.ts` | Yes — auth flow requires Supabase config |
| `EXPO_PUBLIC_SUPABASE_ANON_KEY` | `supabaseClient.ts` | Yes — auth flow requires Supabase config |
| `EXPO_PUBLIC_API_BASE_URL` | `yahooFinanceService.ts` | Recommended — ensures native also uses `/api/quote` fallbacks |
